...
class SearchConfigurationResultEditForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        super(SearchConfigurationResultEditForm, self).__init__(*args, **kwargs)
    ...
    def helper(self):
        ...
        form_row = Div(css_class='form-row', css_id=f'result_field_config_title_row')
        ...
        for row in range(20):
            form_row = Div(css_class='form-row', css_id=f'result_field_config_row_{row}')
            form_field_result_field = Field(f'result_field_{row}', placeholder=f"Search Field {row+1}", wrapper_class=f'col-md-6')
